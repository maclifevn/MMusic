...
const esHangulLib = lazyVar.lazy(async () => import('es-hangul'));
const hanjaLib = lazyVar.lazy(async () => import('hanja'));
const pinyinLib = lazyVar.lazy(async () => import('pinyin-pro'));
const tinyldLib = lazyVar.lazy(async () => import('tinyld'));
...
export const romanizeHangul = async (line: string) => {
  const [esHangul, hanjaMod] = await Promise.all([
    esHangulLib.get(),
    hanjaLib.get(),
  ]);
  const hanja = hanjaMod.default ?? hanjaMod;
  return esHangul.romanize(hanja.translate(line, 'SUBSTITUTION'));
};

export const romanizeChinese = async (line: string) => {
  const { pinyin } = await pinyinLib.get();
...
export const romanizeThai = async (line: string) => {
  const { romanize: romanizeThaiFrag } = await romanizeThaiLib.get();
...
export const romanizeBengali = async (line: string) => {
  try {
    const Sanscript = (await sanscriptLib.get()).default;
...
export const romanizeHindi = async (line: string) => {
  try {
    const Sanscript = (await sanscriptLib.get()).default;
...
export const romanize = async (line: string) => {
  const { detect } = await tinyldLib.get();
  ...
  if (hasKorean([line])) line = await romanizeHangul(line);
  if (hasChinese([line])) line = await romanizeChinese(line);
  if (hasThai([line])) line = await romanizeThai(line);
  if (hasBengali([line])) line = await romanizeBengali(line);
  if (hasHindi([line])) line = await romanizeHindi(line);
