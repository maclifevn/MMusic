...
import { visualizerLoaders } from './visualizers';
...
export default createPlugin({
  ...
  renderer: {
    ...
    async createVisualizer(
      ...
      // Lazily load only the selected visualizer module (butterchurn pulls in
      // ~17MB of presets, so it must not load until actually used).
      const loadToken = Symbol('visualizer-load');
      this.props.pendingLoad = loadToken;
      const visualizerType = await visualizerLoaders[config.type]();
      // A newer createVisualizer() call superseded this one while importing.
      if (this.props.pendingLoad !== loadToken) return;
