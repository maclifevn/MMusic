...
import { languageList } from 'virtual:i18n';
...
import logoOverrideCss from '@/logo-override.css?inline';
...
if (is.linux()) {
  ...
  app.setName('com.mtube.mmusic');
  // for wayland
  app.commandLine.appendSwitch('class', 'com.mtube.mmusic');
}
...
function initTheme(win: BrowserWindow) {
  ...
  injectCSS(win.webContents, logoOverrideCss);
  // Load user CSS
...
async function createMainWindow() {
  ...
  const electronWindowSettings: Electron.BrowserWindowConstructorOptions = {
    ...
    webPreferences: {
      ...
      // YouTube Music has no editable text fields worth spellchecking; disabling
      // it avoids Chromium loading hunspell dictionaries into memory.
      spellcheck: false,
      ...(isTesting()
        ? undefined
        : {
  ...
  // Start loading the page before (not after) loading main plugins so the first
  // paint is not blocked by plugin initialization; plugins load concurrently.
...
const getDefaultLocale = (locale: string) =>
  Object.keys(languageList).includes(locale) ? locale : null;
